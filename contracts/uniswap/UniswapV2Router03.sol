...
contract UniswapV2Router03 is IUniswapV2Router02 {
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        uint deadline
    ) external virtual override ensure(deadline) returns (uint[] memory amounts, uint actualAmountOut) {
        (amounts, actualAmountOut) = UniswapV2Library2.getAmountsOut(
            factory,
            amountIn,
            path,
            msg.sender,
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        bool[] calldata isFOTTokenFlags,
        uint deadline
    ) external virtual override payable ensure(deadline) returns (uint[] memory amounts, uint actualAmountOut) {
        require(path[0] == WETH, 'UniswapV2Router: INVALID_PATH');
    ...
    function swapTokensForExactTokensSupportingFeeOnTransferTokens(
        ...
        amounts = UniswapV2Library2.getAmountsIn(factory, amountOut, path, msg.sender, to, isFOTTokenFlags);
        require(amounts[0] <= amountInMax, 'UniswapV2Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
